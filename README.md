...
**Official implementation of the paper:** *"Error-Guaranteed Compression with Preservation of Downstream Quantities for Electron Microscopy"*
...
1. Clone the repository:
   ```bash
   git clone [https://github.com/jmleeso/STEM_Constraint_Satisfaction.git](https://github.com/jmleeso/STEM_Constraint_Satisfaction.git)
...
* Calibration file: `./data/U100_calibrations.hdf5`
...
We provide a Python script to download the sample datasets directly into the `./data` directory. Run the following command:
...
Run `CS_solver.py` to apply the constraint satisfaction algorithm to your decompressed data. It supports GPU acceleration for fast processing.
...
To compute the fluctuation baseline and validate the statistical error metrics (beta targets) of the decompressed data before and after CS:
